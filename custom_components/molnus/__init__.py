...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    wildlife_required = entry.options.get(
        CONF_WILDLIFE_REQUIRED,
        DEFAULT_WILDLIFE_REQUIRED,
    )
    ...
    # Allow startup even if first refresh fails
    try:
        await coordinator.async_config_entry_first_refresh()
    except Exception as err:
        _LOGGER.warning(
            "Molnus first refresh failed, starting anyway: %s",
            err,
        )
...
async def async_migrate_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    camera_id_slug = slugify(camera_id_raw)
    ...
    desired_entity_id = (
        f"sensor.molnus_camera_latest_image_id_{camera_id_slug}"
    )
